  ...
  return (
    <div className="flex gap-2">
      <Link href={g} target="_blank" rel="noopener noreferrer">
        <button className="h-9 px-3 rounded-2xl bg-brand-700 text-white hover:bg-brand-800">Add to Google</button>
      </Link>
      <Link href={`/api/events/${event.id}/ics`}>
        <button className="h-9 px-3 rounded-2xl border border-line hover:bg-brand-50 text-brand-800">Download .ics</button>
      </Link>
    </div>
  );
}
